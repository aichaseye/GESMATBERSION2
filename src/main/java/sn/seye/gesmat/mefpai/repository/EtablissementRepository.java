...
public interface EtablissementRepository extends JpaRepository<Etablissement, Long> {
    ...
    @Query(
        value = "select distinct etablissement from Etablissement etablissement left join fetch etablissement.localisation left join fetch etablissement.inspection",
    ...
    @Query(
        "select distinct etablissement from Etablissement etablissement left join fetch etablissement.localisation left join fetch etablissement.inspection"
    )
    List<Etablissement> findAllWithToOneRelationships();

    @Query(
        "select etablissement from Etablissement etablissement left join fetch etablissement.localisation left join fetch etablissement.inspection where etablissement.id =:id"
    )
